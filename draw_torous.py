import os
from tkinter import Tk, Frame, Canvas, Menu, messagebox
...
class Donut(Frame):
    def __init__(self, master):
        ...
        self.canvas = Canvas(master, bg='#111111')
    ...
    def create_obj(self):
        for i in range(1600):
            obj = self.canvas.create_text(0, i, font=('Arial', 16))
...
def open_about():
    about_text = (
    "Autoría de la función: JP MathPR en Geogebra.org\n"
    "Programación en Python basada en el trabajo de los desarrolladores: MagnoEfren y ABT\n"
    "Funciones matemáticas:\n"
    "(x - x_{0})^2 + (y - y_{0})^2 = (R + sqrt(r^2 - (z - z_{0})^2))^2\n"
    "(R - sqrt((x - x_{0})^2 + (y - y_{0})^2))^2 + (z - z_{0})^2 = r^2"
)
    messagebox.showinfo("Acerca de...", about_text)

def open_geogebra():
    import webbrowser
    webbrowser.open("https://www.geogebra.org/m/P8ZAnkYG")

def open_github():
    import webbrowser
    webbrowser.open("https://github.com/Odinsoncreator?tab=repositories")
...
if __name__ == '__main__':
    ...
    app = Donut(root)
    icon_path = "C:/Users/baeza/Documents/Dona/assets/InvOpeLogo.ico"
    ...
    about_menu.add_command(label="Información", command=open_about)
    about_menu.add_separator()
    about_menu.add_command(label="Geogebra", command=open_geogebra)
